//! H.264 encoders for LumaLink.
//!
//! Preference order (both via ffmpeg — no Sunshine/Moonlight code):
//! 1. `h264_nvenc` (NVIDIA NVENC)
//! 2. `libx264` ultrafast/zerolatency software fallback
...
pub fn create_encoder(
    ...
    match FfmpegEncoder::spawn(width, height, fps, bitrate_mbps, true) {
        ...
        Err(nvenc_err) => {
            eprintln!("LumaLink: NVENC unavailable ({nvenc_err}); trying libx264");
            let enc = FfmpegEncoder::spawn(width, height, fps, bitrate_mbps, false)?;
            Ok(Box::new(enc))
        }
    }
}
    ...
    fn spawn(
        ...
        let backend = if want_nvenc {
            ...
            if !ffmpeg_has_encoder("libx264") {
                return Err(
                    "ffmpeg를 찾을 수 없거나 libx264/h264_nvenc가 없습니다. ffmpeg를 PATH에 설치해주세요."
                        .into(),
                );
            }
            args.extend([
...
fn read_annex_b(mut stdout: ChildStdout, tx: mpsc::Sender<Vec<u8>>) {
