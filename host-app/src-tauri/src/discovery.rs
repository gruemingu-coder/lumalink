...
//! the LumaLink Streaming *desktop* app (not a plain browser tab —
...
/// LumaLink Streaming app's `src-tauri/src/discovery.rs`.
pub const DISCOVERY_PORT: u16 = 58713;

pub async fn run(state: Arc<SignalingState>) {
    let Ok(socket) = UdpSocket::bind("0.0.0.0:0") else {
        eprintln!("LumaLink discovery broadcaster failed to bind a socket");
        return;
    };
    if socket.set_broadcast(true).is_err() {
        eprintln!("LumaLink discovery broadcaster failed to enable broadcast");
        return;
    }
